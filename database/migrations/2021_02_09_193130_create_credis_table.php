...
class CreateCredisTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('credits', function (Blueprint $table) {
            ...
            $table->foreign('debtor_id')->references('id')->on('clients');
            $table->unsignedBigInteger('first_co_debtor')->nullable()->comment('Codeudor 1');
            $table->foreign('first_co_debtor')->references('id')->on('clients');
            $table->unsignedBigInteger('second_co_debtor')->nullable()->comment('Codeudor 2');
            $table->foreign('second_co_debtor')->references('id')->on('clients');
            $table->date('start_date');
            $table->boolean('refinanced')->default(false);
            $table->double('capital_value');
            $table->double('transport_value')->default(0);
            $table->double('other_value')->default(0);
